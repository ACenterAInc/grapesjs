...
module.exports = Backbone.View.extend({
  ...
  initialize(opts, config) {
    ...
    //this.events[`click.${ppfx}pagetitle`]  = 'click';
  ...
  add(model, fragment) {
    ...
    console.error("TEST MODEL ADD");

    console.log(fragment);
    //model.get('attributes').command = model.attributes.command;
    var view = new BlockView({
      model,
      attributes: model.get('attributes'),
    }, this.config);

//console.error("ATT TEST");
  //  console.error(model.get('attributes'));
    //view.command =model.attributes.command;
    ...
    if(frag) {
      //console.error('adding reg');
      //console.error(rendered);
      frag.appendChild(rendered);
    } else {
      //console.error('adding reg3');
      //console.error(rendered);
